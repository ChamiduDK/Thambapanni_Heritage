...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filter-form, .add-form { margin-bottom: 20px; display: flex; gap: 10px; flex-wrap: wrap; }
        .filter-form input, .add-form input, .add-form select { padding: 12px; border: 2px solid var(--saffron); border-radius: 8px; font-size: 1rem; background: var(--white); transition: var(--transition); color: var(--dark-maroon); max-width: 200px; }
        .filter-form input:focus, .add-form input:focus, .add-form select:focus { border-color: var(--gold); outline: none; box-shadow: 0 0 5px rgba(255, 193, 7, 0.5); }
        .filter-form button, .add-form button { background: var(--green); color: var(--white); padding: 12px 25px; border: none; border-radius: 25px; font-size: 1rem; font-weight: 500; cursor: pointer; transition: var(--transition); }
        .filter-form button:hover, .add-form button:hover { background: var(--gold); color: var(--dark-maroon); transform: translateY(-2px); }
        ...
        @media (max-width: 768px) { .sidebar { width: 200px; left: -200px; } .sidebar.active { left: 0; } .close-sidebar { display: block; } .menu-toggle { display: block; } .main-content { padding: 15px; } h1 { font-size: 1.8rem; } h2 { font-size: 1.5rem; } .filter-form input, .add-form input, .add-form select { max-width: 150px; padding: 10px; font-size: 0.95rem; } .filter-form button, .add-form button { padding: 10px 20px; font-size: 0.95rem; } table { font-size: 0.85rem; } th, td { padding: 8px; } }
        @media (max-width: 480px) { .sidebar { width: 180px; left: -180px; } h1 { font-size: 1.5rem; } h2 { font-size: 1.3rem; } p { font-size: 0.9rem; } .filter-form input, .add-form input, .add-form select { max-width: 120px; padding: 8px; font-size: 0.9rem; } .filter-form button, .add-form button { padding: 8px 15px; font-size: 0.9rem; } table { font-size: 0.8rem; } th, td { padding: 6px; } }
    </style>
</head>
<body>
    <div class="wrapper">
