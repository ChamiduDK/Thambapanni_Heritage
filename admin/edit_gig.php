...
// Fetch gig data
$stmt = $connection->prepare("SELECT * FROM gigs WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $connection->prepare("UPDATE gigs SET title = ?, description = ?, price = ?, category_id = ? WHERE id = ?");
        $stmt->bind_param("ssdii", $title, $description, $price, $category_id, $gig_id);
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = "Error updating gig: " . $connection->error;
        }
        $stmt->close();
    }
}
...
<head>
    ...
    <title>Edit Gig - Thambapanni Heritage</title>
        ...
        <div class="main-content">
            ...
            <div class="content-sections">
                <form method="POST">
                    ...
                    <div>
                        <label>Price:</label>
                        <input type="number" step="0.01" name="price" value="<?php echo htmlspecialchars($gig['price']); ?>" required>
                    </div>
                    <div>
                        <label>Category ID:</label>
                        <input type="number" name="category_id" value="<?php echo htmlspecialchars($gig['category_id']); ?>">
                    </div>
                    <div>
                        <button type="submit">Update Gig</button>
                        <a href="admin_gigs.php" class="back-btn">Back to Gigs</a>
                    </div>
                </form>
            </div>
        </div>
